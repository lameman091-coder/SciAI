...
async def ask_question(request: AskRequest):
    ...
    # 2. SMART RAG
    # Local Context (FAISS)
    faiss_results = search(request.question, top_k=10 if request.book_id else 5, book_id=request.book_id, user_id=request.user_id)
...
async def process_pdf_background(temp_path: str, book_id: str, domain: str, source_type: str, user_id: str):
    try:
        ...
        # 1. Update FAISS
        if chunks:
            add_document(chunks, domain=domain, source_type=source_type, book_id=book_id, user_id=user_id)
...
async def save_article(request: SavedArticleRequest, db: AsyncSession = Depends(get_db)):
    ...
    new_art = SavedArticle(**request.model_dump())
    db.add(new_art)
    await db.commit()
